module.exports = {
    run: function(creep) {
        ...
        let target = Game.getObjectById('58cc52894abb9ae86993937b');
        ...
        if (creep.memory.collecting) {
            ...
            let towers = _.filter(targets, (structure) => structure.structureType === STRUCTURE_TOWER);
            let containersPriority = _.filter(targets, (structure) => structure.structureType === STRUCTURE_CONTAINER && structure.pos.x === 16 && structure.pos.y === 17);
            let containers = _.filter(targets, (structure) => structure.structureType === STRUCTURE_CONTAINER);
            let extensions = _.filter(targets, (structure) => structure.structureType === STRUCTURE_EXTENSION);
            let spawns = _.filter(targets, (structure) => structure.structureType === STRUCTURE_SPAWN);
